...
export class IssuesListInProgressPage {
  ...
  getIssuesList() {
    ...
    load.present();
    this._apiService.callApi(AppSettings.getissuesforuser,'post',{reg_no: this._sharedService.reg_no,type: 'in_progress'}) 
    .subscribe(data => {
      console.log(data);
            if (data.success) {
...
//     })
//     load.present();
// // this._apiService.callApi(AppSettings.issuesListApi, 'post', { mobile: this._sharedService.mobile, reg_no: this._sharedService.reg_no, role: this._sharedService.role,type: 'pending'})
