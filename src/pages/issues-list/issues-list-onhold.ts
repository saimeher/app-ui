...
@Component({
  selector: 'issues-list-onhold',
  templateUrl: 'issues-list-onhold.html',
})
export class IssuesListOnholdPage {
  categories = [];
  issuesList = [];
  display = false;
  collapse: string;
  ...
  getIssuesList() {
    ...
    this._apiService.callApi(AppSettings.getissuesforuser, 'post', { reg_no: this._sharedService.reg_no, type: 'onhold' })
      .subscribe(data => {
        console.log(data);
        if (data.success) {
