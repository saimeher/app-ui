...
@Component({
  selector: 'issues-list-vresolved',
  templateUrl: 'issues-list-vresolved.html',
})
export class IssuesListVerifiedPage {
  categories = [];
  issuesList = [];
  display = false;
  collapse: string;
  ...
  getIssuesList() {
    ...
    load.present();
    this._apiService.callApi(AppSettings.getissuesforuser, 'post', { reg_no: this._sharedService.reg_no, type: 'verified_resolved' })
      .subscribe(data => {
