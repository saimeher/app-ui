...
@Component({
  selector: 'issues-list-cannotbe',
  templateUrl: 'issues-list-cannotbe.html',
})
export class IssuesListCannotbePage {
  categories = [];
  issuesList = [];
  display = false;
  collapse: string;
  ...
  issuesListlength1 = 0;
  issuesListlength2 = 0;
  type1: string ='cannot_be_resolved';
  ...
  getIssuesList() {
    ...
    load.present();
    this._apiService.callApi(AppSettings.getissuesforuser, 'post', { reg_no: this._sharedService.reg_no, type: 'cannot_be_resolved' })
      .subscribe(data => {
